...
class GalleryItem(SQLModel, table=True):
    ...
    # backend/app/main.py `_check_orden_conflict` — because partial unique
    # indexes (`UNIQUE ... WHERE activo = true`) are not portable to vanilla
    # SQLite. The column is a plain int with a Field-level ge=1, le=6 check.
